    ...
    private func setupRequestPublisher() {
        requests
            ...
            .sink { [weak self] completion in
                ...
                self?.requests = PassthroughSubject<AnyPublisher<AuthResponse, Error>, Never>()
                self?.setupRequestPublisher()
            } receiveValue: { [weak self] result in
                self?.state = .authenticated(token: result.token, username: result.user.username, role: result.role)
            }
