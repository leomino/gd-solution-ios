...
class MatchesModel: LoadingStateModel<[Match]> {
    let dataService: MatchDataServiceProtocol
    
    init(state: LoadingState<[Match]> = .idle, dataService: MatchDataServiceProtocol = MatchDataService()) {
        ...
        for await change in changes {
            do {
                ...
                if case .success(var matches) = state {
                    ...
                    matches[correspondingMatchIndex].calculatePoints()
